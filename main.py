...
if len(args) == 1:
    ...
    directory = args[1]
    file_storage = dict()
    for root, dirs, files in os.walk(directory, topdown):  # First, I get each file
        ...
        for name in files:
            ...
            if file_format == "":  # Print every file if file_format is blank
                file_storage[os.path.getsize(os.path.join(root, name)) + hash_key] = [os.path.join(root, name),
                                                                                      hash_key]
    ...
    for key, temp in file_storage.items():  # Now we loop through each file and see which ones are the same
        ...
        for size, value in file_storage.items():  # This one compares each file to the size_compare file for size
            if int(size) - int(value[1]) == size_compare and value[0] not in same_size and value[0] not in seen:
